...
Obtain a token for scripted `caff://` URLs with:

```bash
token="$(caff remote-token)"
open "caff://start?minutes=30&reason=agent&token=${token}"
```
